...
private static NBTTagIntArray getIntArrayTagFor(JsonArray value)
  ...
  for(int i = 0; i < value.size(); i++){array[i]=(int)value.getValue(i).getIntegerValue();}
...
private static NBTTagByteArray getByteArrayTagFor(JsonArray value)
  ...
  for(int i = 0; i < value.size(); i++){array[i]=(byte)value.getValue(i).getIntegerValue();}
  return new NBTTagByteArray(array);
  }

private static NBTTagByte getByteTagFor(JsonValue value){return new NBTTagByte((byte)value.getIntegerValue());}
private static NBTTagShort getShortTagFor(JsonValue value){return new NBTTagShort((short)value.getIntegerValue());}
private static NBTTagInt getIntTagFor(JsonValue value){return new NBTTagInt((int)value.getIntegerValue());}
private static NBTTagLong getLongTagFor(JsonValue value){return new NBTTagLong(value.getIntegerValue());}
private static NBTTagFloat getFloatTagFor(JsonValue value){return new NBTTagFloat((float)value.getFloatValue());}
private static NBTTagDouble getDoubleTagFor(JsonValue value){return new NBTTagDouble(value.getFloatValue());}
private static NBTTagString getStringTagFor(JsonValue value){return new NBTTagString(value.getStringValue());}
