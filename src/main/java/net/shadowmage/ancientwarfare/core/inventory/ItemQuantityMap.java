...
private int count;
private ItemCount(){}
private ItemCount(int qty){count=qty;}
}
...
//  hash = 31*hash + stack.stackSize;//noop for implementation, only care about identity, not quantity
  if(itemTag!=null){hash = 31*hash + itemTag.hashCode();}
  return hash;
  }
...
public boolean equals(Object obj)
  ...
  if(itemTag==null){tagsMatch = wrap.itemTag==null;}
  else if(wrap.itemTag==null){tagsMatch=itemTag==null;}
  else{tagsMatch = itemTag.equals(wrap.itemTag);}  
...
public ItemStack getItemStack()
  ...
  if(cacheStack!=null)
    ...
    if(itemTag!=null){stack.stackTagCompound = (NBTTagCompound)itemTag.copy();}    
    cacheStack = stack;
    return cacheStack;
    }  
  }
...
public static NBTTagCompound writeToNBT(ItemHashEntry entry, NBTTagCompound tag)
  ...
  if(entry.itemTag!=null){tag.setTag("itemTag", entry.itemTag);}
  return tag;
  }
