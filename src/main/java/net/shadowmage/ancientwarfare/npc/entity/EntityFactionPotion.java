...
public class EntityFactionPotion extends EntityThrowable {
	private static final DataParameter<ItemStack> ITEM = EntityDataManager.<ItemStack>createKey(EntityFactionPotion.class, DataSerializers.ITEM_STACK);
	private static final Logger LOGGER = LogManager.getLogger();
	public static final Predicate<EntityLivingBase> WATER_SENSITIVE = new Predicate<EntityLivingBase>() {
		public boolean apply(@Nullable EntityLivingBase p_apply_1_) {
			return EntityFactionPotion.isWaterSensitiveEntity(p_apply_1_);
		}
	};

	private String factionName;

	public EntityFactionPotion(World worldIn) {
		super(worldIn);
	}

	public EntityFactionPotion(World worldIn, EntityLivingBase throwerIn, ItemStack potionDamageIn) {
		super(worldIn, throwerIn);
		this.setItem(potionDamageIn);
	}

	public EntityFactionPotion(World worldIn, EntityLivingBase throwerIn, ItemStack potionDamageIn, String factionName) {
		super(worldIn, throwerIn);
		this.setItem(potionDamageIn);
		this.factionName = factionName;
	}

	public EntityFactionPotion(World worldIn, double x, double y, double z, ItemStack potionDamageIn) {
		super(worldIn, x, y, z);
	...
	private void applySplash(RayTraceResult p_190543_1_, List<PotionEffect> p_190543_2_) {
		...
		if (!list.isEmpty()) {
			for (EntityLivingBase entitylivingbase : list) {
				if (entitylivingbase.canBeHitWithPotion() && !friendlyFactionEntity(entitylivingbase)) {
					double d0 = this.getDistanceSq(entitylivingbase);
	...
	public static void registerFixesPotion(DataFixer fixer) {
		...
		fixer.registerWalker(FixTypes.ENTITY, new ItemStackData(EntityFactionPotion.class, new String[] {"Potion"}));
	}
	...
	private boolean friendlyFactionEntity(EntityLivingBase targetEntity) {
		// only consider an entity friendly if it's a NpcFaction entity from the same faction
		if (targetEntity instanceof NpcFaction) {
			NpcFaction targetFactionEntity = (NpcFaction) targetEntity;
			if (targetFactionEntity.getFaction().equals(factionName)) {
			}
			return targetFactionEntity.getFaction().equals(factionName);
		} else
			return false;
	}
}
