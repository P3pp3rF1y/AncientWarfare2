...
public class AWNPCStatics extends ModConfiguration {
	...
	public void initializeCategories() {
		config.addCustomCategoryComment(generalOptions,
		...
		config.addCustomCategoryComment(serverOptions,
		...
		config.addCustomCategoryComment(clientOptions,
		...
		config.addCustomCategoryComment(recipeSettings,
				"Recipe Options\n" + "Enable / Disable specific recipes, or remove the research requirements from specific recipes.\n" + "Affect only server-side operations.  Will need to be set for dedicated servers, and single\n" + "player (or LAN worlds).  Clients playing on remote servers can ignore these settings.");

		config.addCustomCategoryComment(pathfinderSettings,
		...
		foodConfig.addCustomCategoryComment(foodSettings,
		...
		targetConfig.addCustomCategoryComment(targetSettings,
		...
		targetConfig.addCustomCategoryComment(targetSettingsLegacy,
		...
		factionConfig.addCustomCategoryComment(factionSettings,
	...
	public void initializeValues() {
		...
		maxNpcLevel = config.get(serverOptions, "npc_max_level", maxNpcLevel,
		...
		npcXpFromAttack = config.get(serverOptions, "npc_xp_per_attack", npcXpFromAttack,
		...
		npcXpFromKill = config.get(serverOptions, "npc_xp_per_kill", npcXpFromKill,
				"XP Per Killnefault=" + npcXpFromKill + "\n" + "How much xp should an NPC gain each time they kill an enemy?\n" + "Higher values will result in faster npc leveling.\n" + "Applies to both player-owned and faction-based NPCs.")
				.getInt();

		npcXpFromTrade = config.get(serverOptions, "npc_xp_per_trade", npcXpFromTrade,
		...
		npcXpFromWork = config.get(serverOptions, "npc_xp_per_work", npcXpFromWork,
				"XP Per Work\nDefault=" + npcXpFromWork + "\n" + "How much xp should an NPC gain each time it works at a worksite?\n" + "Higher values will result in faster npc leveling.\n" + "Applies to player-owned NPCs only.")
				.getInt();

		npcXpFromMoveItem = config.get(serverOptions, "npc_xp_per_item_moved", npcXpFromMoveItem,
				"XP Per Courier\nDefault=" + npcXpFromMoveItem + "\n" + "How much xp should an NPC gain each time it moves an item?\n" + "Higher values will result in faster npc leveling.\n" + "Applies to player-owned NPCs only.")
				.getInt();

		npcWorkTicks = config.get(serverOptions, "npc_work_ticks", npcWorkTicks,
		...
		npcAllowUpkeepAnyInventory = config.get(serverOptions, "allow_upkeep_any_inventory", npcAllowUpkeepAnyInventory,
		...
		townMaxRange = config.get(serverOptions, "town_hall_max_range", townMaxRange,
		...
		exportEntityNames = config.get(serverOptions, "export_entity_names", exportEntityNames,
		...
		archerRange = config.get(serverOptions, "archer_attack_range", archerRange,
		...
		npcActionRange = config.get(generalOptions, "npc_action_range", npcActionRange,
		...
		repackCreativeOnly = config.get(generalOptions, "npc_repack_creative_only", repackCreativeOnly,
		...
		persistOrdersOnDeath = config.get(generalOptions, "npc_death_keep_orders_items", persistOrdersOnDeath,
		...
		pathfinderAvoidFences = config.get(pathfinderSettings, "pathfinder_avoid_fences", pathfinderAvoidFences,
		...
		pathfinderAvoidChests = config.get(pathfinderSettings, "pathfinder_avoid_chests", pathfinderAvoidChests,
		...
		PATHFINDER_AVOID_CUSTOM_RAW = config.get(pathfinderSettings, "pathfinder_avoid_others", PATHFINDER_AVOID_CUSTOM_RAW,
	...
	private void loadTargetValues() {
		...
		autoTargetting = targetConfig.get(targetSettings, "auto_targetting", autoTargetting,
		...
		autoTargettingConfigLos = targetConfig.get(targetSettings, "auto_targetting.config.los", autoTargettingConfigLos,
		...
		targets = targetConfig.get(targetSettings, "auto_targetting.exclude", new String[] {EntityRegistry.getEntry(EntityCreeper.class).getName()},
        ...
        */
		...
		if (!autoTargetting) {
			...
			targets = targetConfig.get(targetSettingsLegacy, "enemies_to_target_npcs", defaultTargets,
					"What mob types should have AI inserted to enable them to target NPCs?\n" + "Should work with any new-ai enabled mob type; vanilla or mod-added (but might not work with mod-added entities with custom AI).\n" + "NOTE! This is a LEGACY option! This option ONLY works if the 'auto_inject_mobs' option at the top is changed to false.")
					.getStringList();
			...
			targets = targetConfig.get(targetSettingsLegacy, "combat.archer.targets", defaultTargets, "Default targets for: player-owned archer")
					.getStringList();
			addTargetMapping("combat", "archer", targets);

			targets = targetConfig.get(targetSettingsLegacy, "combat.soldier.targets", defaultTargets, "Default targets for: player-owned soldier")
					.getStringList();
			addTargetMapping("combat", "soldier", targets);

			targets = targetConfig.get(targetSettingsLegacy, "combat.leader.targets", defaultTargets, "Default targets for: player-owned leader npc")
					.getStringList();
			addTargetMapping("combat", "leader", targets);

			targets = targetConfig.get(targetSettingsLegacy, "combat.medic.targets", defaultTargets, "Default targets for: player-owned medic npc")
					.getStringList();
			addTargetMapping("combat", "medic", targets);

			targets = targetConfig.get(targetSettingsLegacy, "combat.engineer.targets", defaultTargets, "Default targets for: player-owned engineer npc")
					.getStringList();
			...
			for (String name : factionNames) {
				for (String sub : factionNpcSubtypes) {
					targets = targetConfig
							.get(targetSettingsLegacy, name + "." + sub + ".targets", defaultTargets, "Default targets for: " + name + " " + asName(sub))
							.getStringList();
	...
	private void loadFoodValues() {
		foodMultiplier = foodConfig.getInt("Food Multiplier", "Default", foodMultiplier, 0, Integer.MAX_VALUE / 10,
				"Food items which don't have a custom duration time set will have their nourishing amount multiplied by this number, to get the number of ticks feeding the npc.");
	...
	private void loadDefaultFactionStandings() {
		...
		for (String name : factionNames) {
			...
			for (String name2 : factionNames) {
				...
				key = name + ":" + name2;
				val = factionConfig.get(factionSettings, key, false,
						"How does: " + name + " faction view: " + name2 + " faction?\n" + "If true, " + name + "s will be hostile towards " + name2 + "s")
						.getBoolean();
	...
	private Attribute getDefault(String type) {
		return new Attribute(valuesConfig.get("01_npc_base_health", type, 20).getDouble(), valuesConfig.get("02_npc_base_attack", type, 1).getDouble(),
				valuesConfig.get("03_npc_base_speed", type, 0.325D).getDouble(), valuesConfig.get("05_npc_base_range", type, 60).getDouble(),
				valuesConfig.get("04_npc_exp_drop", type, 0).getInt());
	}
