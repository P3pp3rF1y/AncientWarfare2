...
public class BlockCoinStackGold extends BlockBaseStructure {
    public BlockCoinStackGold() {
        super(Material.GROUND, "coin_stack_gold");
        setHardness(0.3F);
    ...
    public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
        ...
        coins.setTagCompound(new NBTBuilder().setString("metal","gold").build());
        drops.add(coins);
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (isCoin(stack)) {
            if (coinMetalInHand(stack).equals("gold"))
            {
        ...
        if (!isCoin(stack)) {
            ...
            newStack.setTagCompound(new NBTBuilder().setString("metal","gold").build());
